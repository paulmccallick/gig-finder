...
import { seedLiveScoutFixture } from "./smoke-support/live-scout-fixture";
...
async function liveScoutScreening(stateRoot:string,timeoutMs:number){
  ...
  try{
    ...
    seedLiveScoutFixture(database, { now, positionId, description, descriptionHash, artifactId, descriptionId, profile, profileHash });
    ...
    database.query(`INSERT INTO scout_position_processing(id,position_id,run_id,observation_id,description_id,criteria_id,stage,input_identity,status,created_at,updated_at) VALUES(?,?,'smoke-live-run','smoke-live-observation',?,'src_technology_leadership_v1','screen_relevance',?,'pending',?,?)`).run(processingId,positionId,descriptionId,relevanceIdentity,now,now);
