...
export interface DocumentReadRepository {
  ...
  list(ownerType: DocumentOwnerType, ownerId: string): ManagedDocumentRecord[];
...
export interface ArtifactPort{personProfile(personId:string):Promise<string>;jobDescription(jobId:string):Promise<string>;interviewPrep(jobId:string):Promise<{name:string;content:string}[]>;personProfileExists(personId:string):Promise<boolean>;jobDescriptionExists(jobId:string):Promise<boolean>;interviewPrepExists(jobId:string):Promise<boolean>;verify(expectations:{people:{id:string;hasLocalProfile:boolean}[];jobs:{id:string;hasJobDescription:boolean;hasInterviewPrep:boolean}[]}):Promise<ArtifactVerification>}
