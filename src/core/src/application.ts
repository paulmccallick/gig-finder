...
import { ArtifactDomainService, ContactDomainService, GigDomainService, TaskDomainService } from "./tracker-services";
...
export class GigFinderApplication {
  ...
  constructor(persistence:Persistence,audit:AuditPort,artifacts:ArtifactPort){
    ...
    this.documents=new ManagedDocumentService(persistence);this.gigs=new GigDomainService(persistence,artifacts,changeExecutor,this.documents);this.people=new PeopleService(persistence);this.networking=new ContactDomainService(persistence,changeExecutor,this.documents);this.tasks=new TaskDomainService(persistence);this.meetings=new MeetingService(persistence,this.gigs,this.people,changeExecutor);this.gigPeople=new GigPeopleService(persistence,this.gigs,this.people);this.events=new EventService(persistence);this.history=new HistoryService(audit);this.changes=new ChangeService(persistence);this.artifacts=new ArtifactDomainService(persistence,artifacts);
    this.documentReader=new ApplicationDocumentReader({gigs:this.gigs,contacts:this.networking,managed:this.documents});
    this.contextSearch=new SearchContextService({gigs:this.gigs,networking:this.networking});
  }
}
