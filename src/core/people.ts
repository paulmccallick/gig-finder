...
const personMutableFields={name:z.string().trim().min(1).describe("Person name."),company:personNullableTextSchema.describe("Company, or null."),title:personNullableTextSchema.describe("Title, or null."),linkedInProfileUrl:z.string().url().nullable().describe("LinkedIn profile URL, or null."),connectedOn:personDateSchema.nullable().describe("Connection date, or null."),relationship:personRelationshipSchema.describe("Relationship details."),priority:z.enum(personPriorities).describe("Relationship priority."),status:z.enum(personStatuses).describe("Relationship workflow status."),whyInteresting:personNullableTextSchema.describe("Why this person matters, or null."),notes:z.array(z.string()).describe("Replacement notes list."),tags:z.array(z.string().trim().min(1)).describe("Replacement tags list.")};
export const personEntitySchema=z.object({id:z.string().trim().min(1),...personMutableFields,lastContacted:personDateSchema.nullable().describe("Derived latest completed Interaction date, or null."),lastContactMethod:personNullableTextSchema.describe("Derived latest completed Interaction channel, or null."),lastContactSummary:personNullableTextSchema.describe("Derived latest completed Interaction summary, or null."),profileStatus:z.enum(["missing","verified"]),createdAt:personDateSchema,updatedAt:personDateSchema}).strict();
export type Person=z.infer<typeof personEntitySchema>;
export const personInputSchema=z.object({name:personMutableFields.name.optional(),company:personMutableFields.company.optional(),title:personMutableFields.title.optional(),linkedInProfileUrl:personMutableFields.linkedInProfileUrl.optional(),connectedOn:personMutableFields.connectedOn.optional(),relationship:z.object({type:personRelationshipSchema.shape.type.optional(),strength:personRelationshipSchema.shape.strength.optional(),introducedBy:personRelationshipSchema.shape.introducedBy.optional(),notes:personRelationshipSchema.shape.notes.optional()}).strict().refine(value=>Object.keys(value).length>0,"Relationship input must contain at least one field.").optional().describe("Relationship details."),priority:personMutableFields.priority.optional(),status:personMutableFields.status.optional(),whyInteresting:personMutableFields.whyInteresting.optional(),notes:personMutableFields.notes.optional(),tags:personMutableFields.tags.optional()}).strict().refine(value=>Object.keys(value).length>0,"Person input must contain at least one field.");
export type PersonInput=z.infer<typeof personInputSchema>;
export const personInputFieldPaths=["name","company","title","linkedInProfileUrl","connectedOn","relationship.type","relationship.strength","relationship.introducedBy","relationship.notes","priority","status","whyInteresting","notes","tags"] as const;
export const personClearableInputFieldPaths=new Set<typeof personInputFieldPaths[number]>(["company","title","linkedInProfileUrl","connectedOn","relationship.introducedBy","relationship.notes","whyInteresting"]);
