...
export class ScoutPositionService {
 constructor(
  ...
  private readonly documents:Pick<ManagedDocumentService,"create"|"get"|"createdByChange">,
 ){}
 ...
 private promote(positionId:string,now:string):ScoutPositionDetail|null {
  try {
   ...
   this.gigs.createNew({actor:work.actor,source:"automation",summary:"Promote Scout position to Gig",changeId:`${work.changeId}:gig`,occurredAt:now},gigId,gig);
   const documentChangeId=`${work.changeId}:document`;
   const existing=this.documents.createdByChange(documentChangeId);
   const document=existing??this.documents.create({actor:work.actor,source:"automation",summary:"Create reviewed Scout job description",changeId:documentChangeId,occurredAt:now},{links:[{entityType:"gig",entityId:gigId}],documentType:"job_description",title:`${work.company} — ${work.title}`,mediaType:"text/markdown",sourceDescription:work.sourceDescription,content:work.markdown,uploadProvenance:null}).document;
   if(document.content!==work.markdown||!document.links.some(link=>link.entityType==="gig"&&link.entityId===gigId))throw new Error("Reviewed Scout document replay does not match the persisted promotion.");
   this.store.completePromotion(positionId,gigId,document.id,now);
  } catch(reason) {
