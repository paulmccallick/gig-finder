...
export function normalizeDescription(value: unknown): string | null {
  ...
  const markdownish = /<[a-z][\s\S]*>/i.test(value)
    ? value
        .replace(/<\s*br\s*\/?\s*>/gi, "\n")
...
export async function retrieveOfficialDescription(
  url: string,
  ...
  if (parsed.protocol !== "https:")
    throw new Error("Official description URLs must use HTTPS.");
  ...
  if (response.status < 200 || response.status >= 300)
    throw new Error(`description_http_${response.status}`);
