...
import type { AuditPort,DocumentWriteRepository,Persistence,ReadRepository,UnitOfWork } from "../ports";
...
class DocumentRepo implements DocumentWriteRepository {
  ...
  create(input:{document:ManagedDocumentData;content:string;contentHash:string;sourceProvenance?:ManagedDocumentSourceProvenance}){const record={...input.document,displayName:documentDisplayName(input.document),currentVersion:1,content:input.content,contentHash:input.contentHash,createdAt:metadata.createdAt,updatedAt:metadata.updatedAt};this.rows.set(record.id,record);this.histories.set(record.id,[{documentId:record.id,version:1,parentVersion:null,content:record.content,contentHash:record.contentHash,changeId:"test-change",changeSummary:"change",createdAt:record.createdAt,createdBy:"test",sourceDescription:input.sourceProvenance?record.sourceDescription:null,sourceProvenance:input.sourceProvenance??null}]);return record}
...
const audit:AuditPort={query:query=>({query})};const app=new GigFinderApplication(persistence,audit);const context:ChangeContext={actor:"test",source:"test",summary:"change"};
...
const seedGig=(id:string,overrides:Partial<GigSummary>={}):ReturnType<typeof app.gigs.get>=>{
 app.gigs.create(context,{id,company:"Synthetic Existing Company",title:"Synthetic Existing Role",externalJobId:null,stage:"identified",outcome:"pending",statusSummary:"Synthetic existing opportunity",lastActivity:"2026-08-20",nextAction:null,fit:{rating:"good",summary:null},payRange:null,sourceUrl:null,tags:[],availability:"unknown",availabilityUpdatedAt:null,location:null,workArrangement:null,postedDate:null,businessUnitTeam:null,recruiterSource:null,bonus:null,equity:null,otherCompensation:null,...overrides});
...
describe("application services",()=>{
 ...
 test("Gig availability changes through an audited mutation and ignores an unchanged value",()=>{
  app.gigs.create(context,{id:"gig-1",company:"Availability Company",title:"Director",externalJobId:null,stage:"identified",outcome:"pending",statusSummary:"Found",lastActivity:"2026-08-20",nextAction:null,fit:{rating:"good",summary:null},payRange:null,sourceUrl:null,tags:[]});
 ...
 test("Gig availability rejects an invalid occurredAt before changing its record or audit",()=>{
  app.gigs.create(context,{id:"gig-invalid-occurred-at",company:"Availability Company",title:"Director",externalJobId:null,stage:"identified",outcome:"pending",statusSummary:"Found",lastActivity:"2026-08-20",nextAction:null,fit:{rating:"good",summary:null},payRange:null,sourceUrl:null,tags:[]});
 ...
 test("gig-people service filters relationships",()=>{if(!app.gigs.get("gig"))seedGig("gig");if(!app.people.get("person"))app.people.create(context,{id:"person",name:"Person",company:null,title:null,linkedInProfileUrl:null,connectedOn:null});app.gigPeople.create(context,{id:"relation",gigId:"gig",personId:"person",relationship:"interviewer",notes:null});const result=app.gigPeople.query({gigIds:["gig"]});expect(result.status==="ok"?result.items:[]).toHaveLength(1)});
 ...
 test("managed document service creates immutable versions and deduplicates identical content",async()=>{
  if(!app.gigs.get("gig"))app.gigs.create(context,{id:"gig",company:"Company",title:"VP",externalJobId:null,stage:"identified",outcome:"pending",statusSummary:"Found",lastActivity:"2026-07-22",nextAction:null,fit:{rating:"good",summary:null},payRange:null,sourceUrl:null,tags:[]});
  ...
  expect(discovery).toMatchObject({status:"ok",page:{limit:50,total:1}});
