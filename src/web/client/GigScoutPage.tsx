...
function PositionsWorkspace(){
 ...
 const decide=async(action:"irrelevant"|"defer"|"pursue")=>{if(!detail?.descriptionId||!detail.relevanceEvaluationId||!detail.candidateMatchEvaluationId)return;const response=await fetch(`/api/gig-scout/positions/${encodeURIComponent(detail.id)}/decision`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({changeId:`change_${crypto.randomUUID()}`,actor:"User",action,note:note||undefined,reviewAt:action==="defer"?new Date(reviewAt).toISOString():undefined,expectedStateRevision:detail.stateRevision,descriptionId:detail.descriptionId,relevanceEvaluationId:detail.relevanceEvaluationId,candidateMatchEvaluationId:detail.candidateMatchEvaluationId})});if(!response.ok){const value=await response.json() as {error?:string};setError(value.error??"Could not save position decision.");return;}setItems(values=>values.filter(value=>value.id!==detail.id));setTotal(value=>Math.max(0,value-1));setSelected(null);setNote("");};
