...
const boundedPattern = z
  ...
  .refine(
    ...
    "Pattern uses unsupported backtracking constructs.",
  );

export const jsonSourceSchema = commonSource
  .extend({
    type: z.literal("json"),
...
export const platformSourceSchema = commonSource
  .extend({
    type: z.literal("platform"),
    adapter: z.enum(platformAdapters),
    searchTerms: z.array(z.string().trim().min(1).max(100)).max(25).default([]),
  })
  .strict();

export const sourceConfigurationSchema = z.discriminatedUnion("type", [
  jsonSourceSchema,
  htmlSourceSchema,
...
export interface SourceAttempt {
  adapter: SourceConfiguration["type"];
  stage: string;
  requestCount: number;
