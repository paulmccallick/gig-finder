...
export function extractJson(source: Extract<SourceConfiguration, { type: "json" }>, body: string): { positions: NormalizedPosition[]; hasNext: boolean; surfaceVerified: boolean } {
  const parsed: unknown = JSON.parse(body);
  const records = atPath(parsed, source.recordsPath);
  if (!Array.isArray(records)) return { positions: [], hasNext: false, surfaceVerified: false };
  const positions = records.flatMap((record): NormalizedPosition[] => {
    if (!record || typeof record !== "object") return [];
    const read = (path: string | undefined) => path ? atPath(record, path) : undefined;
    const title = read(source.fields.title); const rawUrl = read(source.fields.url);
