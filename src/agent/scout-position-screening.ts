...
export class AiSdkScoutScreeningModel implements ScoutScreeningModel {
 ...
 async screenRelevance(input:RelevanceRequest):Promise<ModelResult<RelevanceResult>>{const started=Date.now();const result=streamText({model:await this.resolve(),instructions:scoutRelevanceInstructions,prompt:modelPrompt(relevanceModelPayload(input)),output:Output.object({schema:relevanceResultSchema,name:"scout_relevance_result"}),abortSignal:AbortSignal.timeout(60_000),maxRetries:1,providerOptions:{openai:{store:false}}});const[value,usage]=await Promise.all([result.output,result.usage]);return{value,metrics:this.metrics(started,usage)};}
 async scoreCandidateMatch(input:CandidateMatchRequest):Promise<ModelResult<CandidateMatchResult>>{const started=Date.now(),promptCacheKey=await candidateMatchPromptCacheKey(input);const result=streamText({model:await this.resolve(),instructions:scoutCandidateMatchInstructions,prompt:modelPrompt(candidateMatchModelPayload(input)),output:Output.object({schema:candidateMatchResultSchema,name:"scout_candidate_match_result"}),abortSignal:AbortSignal.timeout(60_000),maxRetries:1,providerOptions:{openai:{store:false,promptCacheKey}}});const[value,usage]=await Promise.all([result.output,result.usage]);return{value,metrics:this.metrics(started,usage)};}
}
