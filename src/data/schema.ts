...
export const scoutPositionStates=sqliteTable("scout_position_states",{positionId:text("position_id").primaryKey().references(()=>scoutPositions.id),state:text("state",{enum:["processing","needs_user_review","irrelevant","rejected","deferred","promoted"]}).notNull(),linkedGigId:text("linked_gig_id").references(()=>gigs.id),deferredUntil:text("deferred_until"),revision:integer("revision").notNull().default(1),createdAt:text("created_at").notNull(),updatedAt:text("updated_at").notNull()});
