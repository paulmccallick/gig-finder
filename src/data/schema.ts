...
export const scoutRuns=sqliteTable("scout_runs",{id:text("id").primaryKey(),status:text("status").notNull(),runType:text("run_type").notNull().default("full"),sourceRunId:text("source_run_id"),operatorReason:text("operator_reason"),requestFingerprint:text("request_fingerprint"),batchSize:integer("batch_size").notNull(),concurrency:integer("concurrency").notNull(),searchProfileJson:text("search_profile_json").notNull().default('{"terms":[],"locations":[]}'),screeningCacheKey:text("screening_cache_key"),candidateProfileJson:text("candidate_profile_json"),candidateProfileVersion:text("candidate_profile_version"),candidateProfileArtifactId:text("candidate_profile_artifact_id"),candidateProfileHash:text("candidate_profile_hash"),screeningModel:text("screening_model"),screeningProvider:text("screening_provider"),screeningModelConfiguration:text("screening_model_configuration"),createdAt:text("created_at").notNull(),startedAt:text("started_at"),completedAt:text("completed_at"),companyCount:integer("company_count").notNull().default(0),succeededCount:integer("succeeded_count").notNull().default(0),failedCount:integer("failed_count").notNull().default(0)},table=>[
 uniqueIndex("scout_runs_one_active_full_idx").on(table.runType).where(sql`${table.runType}='full' AND ${table.status} IN ('queued','running')`),
 uniqueIndex("scout_runs_legacy_backfill_source_idx").on(table.sourceRunId).where(sql`${table.runType}='legacy_backfill'`),
...
export const scoutRunCompanies=sqliteTable("scout_run_companies",{id:text("id").primaryKey(),runId:text("run_id").notNull().references(()=>scoutRuns.id),companyId:text("company_id").notNull().references(()=>scoutCompanies.id),companyName:text("company_name").notNull(),companyConfigurationId:text("company_configuration_id").notNull().references(()=>scoutCompanyConfigurations.id),status:text("status").notNull(),startedAt:text("started_at"),completedAt:text("completed_at"),failureCode:text("failure_code"),failureMessage:text("failure_message")});
...
export const scoutPositionDescriptions=sqliteTable("scout_position_descriptions",{id:text("id").primaryKey(),positionId:text("position_id").notNull().references(()=>scoutPositions.id),artifactId:text("artifact_id").notNull().references(()=>scoutDescriptionArtifacts.id),sourceUrl:text("source_url").notNull(),retrievedAt:text("retrieved_at").notNull(),sourceContentHash:text("source_content_hash").notNull(),markdownContentHash:text("markdown_content_hash").notNull(),converterVersion:text("converter_version").notNull(),createdAt:text("created_at").notNull()},table=>[uniqueIndex("scout_position_description_identity_idx").on(table.positionId,table.markdownContentHash,table.converterVersion)]);
...
export const scoutPositionStates=sqliteTable("scout_position_states",{positionId:text("position_id").primaryKey().references(()=>scoutPositions.id),state:text("state",{enum:["processing","needs_user_review","irrelevant","rejected","deferred","promoted"]}).notNull(),linkedGigId:text("linked_gig_id").references(()=>gigs.id),deferredUntil:text("deferred_until"),currentDecisionId:text("current_decision_id"),revision:integer("revision").notNull().default(1),createdAt:text("created_at").notNull(),updatedAt:text("updated_at").notNull()});
export const scoutPositionProcessing=sqliteTable("scout_position_processing",{id:text("id").primaryKey(),positionId:text("position_id").notNull().references(()=>scoutPositions.id),runId:text("run_id").references(()=>scoutRuns.id),observationId:text("observation_id").references(()=>scoutPositionObservations.id),configurationSourceId:text("configuration_source_id").references(()=>scoutCompanyConfigurationSources.id),descriptionId:text("description_id").references(()=>scoutPositionDescriptions.id),criteriaId:text("criteria_id").references(()=>scoutRelevanceCriteria.id),relevanceEvaluationId:text("relevance_evaluation_id").references(()=>scoutRelevanceEvaluations.id),rubricId:text("rubric_id").references(()=>scoutCandidateMatchRubrics.id),stage:text("stage",{enum:["reconcile_gig","acquire_description","screen_relevance","score_candidate_match"]}).notNull(),inputIdentity:text("input_identity").notNull(),status:text("status",{enum:["pending","completed","failed","superseded"]}).notNull(),attemptCount:integer("attempt_count").notNull().default(0),failureCode:text("failure_code"),failureMessage:text("failure_message"),documentProjectionStatus:text("document_projection_status",{enum:["pending","updated","unchanged","failed"]}),createdAt:text("created_at").notNull(),updatedAt:text("updated_at").notNull(),completedAt:text("completed_at")},table=>[uniqueIndex("scout_position_processing_identity_idx").on(table.positionId,table.stage,table.inputIdentity),index("scout_position_processing_work_idx").on(table.stage,table.status,table.updatedAt,table.id),check("scout_position_processing_document_projection_status_check",sql`${table.documentProjectionStatus} is null or ${table.documentProjectionStatus} in ('pending','updated','unchanged','failed')`)]);
export const scoutDescriptionAcquisitions=sqliteTable("scout_description_acquisitions",{processingId:text("processing_id").primaryKey().references(()=>scoutPositionProcessing.id),descriptionId:text("description_id").notNull().references(()=>scoutPositionDescriptions.id),sourceUrl:text("source_url").notNull(),retrievedAt:text("retrieved_at").notNull(),sourceContentHash:text("source_content_hash").notNull(),extractedContentHash:text("extracted_content_hash").notNull(),sourceKey:text("source_key").notNull(),configurationVersion:integer("configuration_version").notNull(),extractionStrategy:text("extraction_strategy").notNull(),converterVersion:text("converter_version").notNull(),createdAt:text("created_at").notNull()},table=>[index("scout_description_acquisitions_description_idx").on(table.descriptionId,table.processingId)]);
export const scoutPositionProcessingOutbox=sqliteTable("scout_position_processing_outbox",{id:text("id").primaryKey(),processingId:text("processing_id").notNull().unique().references(()=>scoutPositionProcessing.id),queueJobId:text("queue_job_id").notNull().unique(),dispatchStatus:text("dispatch_status",{enum:["pending","dispatched"]}).notNull().default("pending"),createdAt:text("created_at").notNull(),dispatchedAt:text("dispatched_at")});
export const scoutPositionBackfillItems=sqliteTable("scout_position_backfill_items",{runId:text("run_id").notNull().references(()=>scoutRuns.id),positionId:text("position_id").notNull().references(()=>scoutPositions.id),observationId:text("observation_id").notNull().references(()=>scoutPositionObservations.id),configurationSourceId:text("configuration_source_id").notNull().references(()=>scoutCompanyConfigurationSources.id),linkedGigId:text("linked_gig_id").references(()=>gigs.id),companyName:text("company_name"),templateName:text("template_name"),initialState:text("initial_state"),initialDecisionOrigin:text("initial_decision_origin"),descriptionOutcome:text("description_outcome",{enum:["corrected","unchanged"]}),finalOutcome:text("final_outcome",{enum:["agent_irrelevant","agent_irrelevant_to_review","needs_user_review","promoted","user_workflow_preserved","failed","unavailable","superseded"]}),failureCode:text("failure_code"),requestedAt:text("requested_at").notNull(),completedAt:text("completed_at")},table=>[primaryKey({columns:[table.runId,table.positionId]}),index("scout_position_backfill_items_position_idx").on(table.positionId,table.runId),check("scout_position_backfill_items_description_outcome_check",sql`${table.descriptionOutcome} is null or ${table.descriptionOutcome} in ('corrected','unchanged')`),check("scout_position_backfill_items_final_outcome_check",sql`${table.finalOutcome} is null or ${table.finalOutcome} in ('agent_irrelevant','agent_irrelevant_to_review','needs_user_review','promoted','user_workflow_preserved','failed','unavailable','superseded')`)]);
export const scoutRelevanceCriteria=sqliteTable("scout_relevance_criteria",{id:text("id").primaryKey(),version:integer("version").notNull().unique(),criteria:text("criteria").notNull(),confidenceThreshold:integer("confidence_threshold").notNull(),promptVersion:text("prompt_version").notNull(),createdAt:text("created_at").notNull()});
export const scoutCandidateMatchRubrics=sqliteTable("scout_candidate_match_rubrics",{id:text("id").primaryKey(),version:integer("version").notNull().unique(),rubric:text("rubric").notNull(),promptVersion:text("prompt_version").notNull(),createdAt:text("created_at").notNull()});
export const scoutRelevanceEvaluations=sqliteTable("scout_relevance_evaluations",{id:text("id").primaryKey(),positionId:text("position_id").notNull().references(()=>scoutPositions.id),descriptionId:text("description_id").notNull().references(()=>scoutPositionDescriptions.id),criteriaId:text("criteria_id").notNull().references(()=>scoutRelevanceCriteria.id),inputIdentity:text("input_identity").notNull().unique(),decision:text("decision").notNull(),reason:text("reason").notNull(),confidence:integer("confidence").notNull(),evidenceJson:text("evidence_json").notNull(),ambiguitiesJson:text("ambiguities_json").notNull(),provider:text("provider").notNull(),model:text("model").notNull(),modelConfiguration:text("model_configuration").notNull(),inputTokens:integer("input_tokens"),outputTokens:integer("output_tokens"),cacheReadTokens:integer("cache_read_tokens"),cacheWriteTokens:integer("cache_write_tokens"),latencyMs:integer("latency_ms").notNull(),createdAt:text("created_at").notNull()});
export const scoutCandidateMatchEvaluations=sqliteTable("scout_candidate_match_evaluations",{id:text("id").primaryKey(),positionId:text("position_id").notNull().references(()=>scoutPositions.id),relevanceEvaluationId:text("relevance_evaluation_id").notNull().references(()=>scoutRelevanceEvaluations.id),inputIdentity:text("input_identity").notNull().unique(),profileVersion:text("profile_version").notNull(),profileArtifactId:text("profile_artifact_id").notNull(),profileHash:text("profile_hash").notNull(),rubricId:text("rubric_id").notNull().references(()=>scoutCandidateMatchRubrics.id),score:integer("score").notNull(),scoreExplanation:text("score_explanation").notNull(),provider:text("provider").notNull(),model:text("model").notNull(),modelConfiguration:text("model_configuration").notNull(),inputTokens:integer("input_tokens"),outputTokens:integer("output_tokens"),cacheReadTokens:integer("cache_read_tokens"),cacheWriteTokens:integer("cache_write_tokens"),latencyMs:integer("latency_ms").notNull(),createdAt:text("created_at").notNull()});
export const scoutPositionDecisions=sqliteTable("scout_position_decisions",{id:text("id").primaryKey(),changeId:text("change_id").notNull().unique().references(()=>changes.id),positionId:text("position_id").notNull().references(()=>scoutPositions.id),action:text("action",{enum:["irrelevant","defer","restore","pursue","reverse"]}).notNull(),origin:text("origin",{enum:["agent","user","system"]}).notNull(),actor:text("actor").notNull(),reason:text("reason"),note:text("note"),descriptionId:text("description_id").references(()=>scoutPositionDescriptions.id),relevanceEvaluationId:text("relevance_evaluation_id").references(()=>scoutRelevanceEvaluations.id),candidateMatchEvaluationId:text("candidate_match_evaluation_id").references(()=>scoutCandidateMatchEvaluations.id),expectedStateRevision:integer("expected_state_revision").notNull(),resultingStateRevision:integer("resulting_state_revision").notNull(),reviewAt:text("review_at"),reversesDecisionId:text("reverses_decision_id").references(():AnySQLiteColumn=>scoutPositionDecisions.id),createdAt:text("created_at").notNull()},table=>[index("scout_position_decisions_position_idx").on(table.positionId,table.createdAt,table.id)]);
export const scoutPositionNotes=sqliteTable("scout_position_notes",{id:text("id").primaryKey(),positionId:text("position_id").notNull().references(()=>scoutPositions.id),decisionId:text("decision_id").references(()=>scoutPositionDecisions.id),actor:text("actor").notNull(),body:text("body").notNull(),createdAt:text("created_at").notNull()},table=>[index("scout_position_notes_position_idx").on(table.positionId,table.createdAt,table.id)]);
...
export const managedDocumentVersions = sqliteTable("managed_document_versions", {
  ...
  sourceDescription: text("source_description"),
  sourceProvenanceJson: text("source_provenance_json"),
}, (table) => [
