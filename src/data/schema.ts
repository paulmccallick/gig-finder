...
export const scoutPositionProcessing=sqliteTable("scout_position_processing",{id:text("id").primaryKey(),positionId:text("position_id").notNull().references(()=>scoutPositions.id),stage:text("stage",{enum:["reconcile_gig"]}).notNull(),inputIdentity:text("input_identity").notNull(),status:text("status",{enum:["pending","completed","failed","superseded"]}).notNull(),attemptCount:integer("attempt_count").notNull().default(0),failureCode:text("failure_code"),failureMessage:text("failure_message"),createdAt:text("created_at").notNull(),updatedAt:text("updated_at").notNull(),completedAt:text("completed_at")},table=>[uniqueIndex("scout_position_processing_identity_idx").on(table.positionId,table.stage,table.inputIdentity),index("scout_position_processing_work_idx").on(table.stage,table.status,table.updatedAt,table.id)]);
