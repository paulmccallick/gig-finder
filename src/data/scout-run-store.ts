...
import type { ScoutBackfillStatus,ScoutPositionDetail,ScoutPositionProcessingJob,ScoutPositionProcessingStage,ScoutPositionStore,ScoutUserDecisionCommand,ScoutWorkspacePage } from "../core/scout/engine/positions";
...
import type { GigDomainService } from "../core/tracker-services";
import type { ManagedDocumentService } from "../core/documents";
...
export interface ScoutPromotionServices {gigs:Pick<GigDomainService,"createNew">;documents:Pick<ManagedDocumentService,"create"|"get">}
export class SqliteScoutRunStore implements ScoutRunStore,ScoutPositionStore,ScoutPositionProcessingRepository {
  constructor(
    ...
    private readonly promotionServices?:ScoutPromotionServices,
  ) {}
  startOrReuse(
    batchSize: number,
    concurrency: number,
  ...
  private promote(positionId:string,descriptionId:string,changeId:string,actor:string,now:string){
    const source=this.db.query(`SELECT p.title,p.location,p.external_id externalId,p.canonical_url canonicalUrl,c.name company,d.source_url sourceUrl,d.retrieved_at retrievedAt,d.markdown_content_hash contentHash,a.file_path filePath,a.provenance_json provenance FROM scout_positions p JOIN scout_companies c ON c.id=p.company_id JOIN scout_position_descriptions d ON d.id=? AND d.position_id=p.id JOIN scout_description_artifacts a ON a.id=d.artifact_id WHERE p.id=?`).get(descriptionId,positionId) as Record<string,unknown>|null;
    if(!source||!this.descriptionsRoot||!this.promotionServices)throw new Error("Reviewed Scout description promotion is unavailable.");
    const markdown=readFileSync(path.resolve(this.descriptionsRoot,String(source.filePath)),"utf8"),gigId=id("gig","scout",positionId),promotionId=id("spprom",positionId),gigChangeId=`${changeId}:gig`,documentChangeId=`${changeId}:document`;
    this.promotionServices.gigs.createNew({actor,source:"automation",summary:"Promote Scout position to Gig",changeId:gigChangeId,occurredAt:now},gigId,{company:String(source.company),title:String(source.title),externalJobId:nullableText(source.externalId),stage:"identified",outcome:"pending",statusSummary:"Promoted from Gig Scout",lastActivity:now.slice(0,10),nextAction:null,fit:{rating:"tbd",summary:null},payRange:null,sourceUrl:String(source.sourceUrl??source.canonicalUrl),tags:[],location:nullableText(source.location),workArrangement:null,postedDate:null,businessUnitTeam:null,recruiterSource:null,bonus:null,equity:null,otherCompensation:null});
    const existingDocument=this.db.query(`SELECT document_id documentId FROM managed_document_versions WHERE change_id=?`).get(documentChangeId) as {documentId:string}|null;
    const provenance=JSON.parse(String(source.provenance)) as Record<string,unknown>;
    const sourceDescription=JSON.stringify({scoutDescriptionId:descriptionId,officialSourceUrl:String(source.sourceUrl??source.canonicalUrl),retrievedAt:String(source.retrievedAt),sourceKey:provenance.sourceKey,configurationVersionId:provenance.configurationVersionId,extractionStrategy:provenance.extractionStrategy,converterVersion:provenance.converterVersion});
    const document=existingDocument?this.promotionServices.documents.get(existingDocument.documentId):this.promotionServices.documents.create({actor,source:"automation",summary:"Create reviewed Scout job description",changeId:documentChangeId,occurredAt:now},{links:[{entityType:"gig",entityId:gigId}],documentType:"job_description",title:`${source.company} — ${source.title}`,mediaType:"text/markdown",sourceDescription,content:markdown,uploadProvenance:null}).document;
    if(!document||document.content!==markdown||!document.links.some(link=>link.entityType==="gig"&&link.entityId===gigId))throw new Error("Reviewed Scout document replay does not match the persisted promotion.");
    const documentId=document.id;
    this.db.transaction(()=>{
      this.db.query(`UPDATE scout_position_promotions SET gig_id=?,managed_document_id=?,status='completed',attempt_count=attempt_count+1,updated_at=?,completed_at=? WHERE id=?`).run(gigId,documentId,now,now,promotionId);
      this.db.query(`UPDATE scout_position_states SET state='promoted',linked_gig_id=?,deferred_until=NULL,updated_at=? WHERE position_id=?`).run(gigId,now,positionId);
    })();
  }
  retryPromotion(positionId:string,now:string):ScoutPositionDetail|null{const promotion=this.db.query(`SELECT p.description_id descriptionId,d.change_id changeId,d.actor FROM scout_position_promotions p JOIN scout_position_decisions d ON d.id=p.decision_id WHERE p.position_id=? AND p.status IN ('pending','failed')`).get(positionId) as {descriptionId:string;changeId:string;actor:string}|null;if(!promotion)return this.reviewDetail(positionId);try{this.promote(positionId,promotion.descriptionId,promotion.changeId,promotion.actor,now);}catch(reason){const message=reason instanceof Error?reason.message:"Promotion failed.";this.db.query(`UPDATE scout_position_promotions SET status='failed',failure_code='promotion_failed',failure_message=?,attempt_count=attempt_count+1,updated_at=? WHERE position_id=? AND status<>'completed'`).run(message.slice(0,500),now,positionId);}return this.reviewDetail(positionId);}
