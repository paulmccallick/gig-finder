...
export class SqliteScoutRunStore implements ScoutRunStore,ScoutPositionStore,ScoutPostingResolutionStore,ScoutPositionProcessingRepository {
  ...
  startOrReuse(
    ...
    return this.db.transaction(() => {
      ...
      const companies = this.db
        .query(
          `SELECT id,name,current_configuration_id FROM scout_companies WHERE active=1 ORDER BY id`,
        )
        .all() as Array<{ id: string; name: string; current_configuration_id: string }>;
      this.db
        .query(
      ...
      for (const company of companies) {
        ...
        this.db
          .query(
            `INSERT INTO scout_run_companies(id,run_id,company_id,company_name,company_configuration_id,status) VALUES(?,?,?,?,?,'queued')`,
          )
          .run(
            runCompanyId,
            runId,
  ...
  private processingInputIdentity(positionId:string,observationId?:string){
    const row=this.db.query(`SELECT p.external_id,coalesce(o.canonical_url,p.canonical_url) canonical_url,o.title,o.location FROM scout_positions p LEFT JOIN scout_position_observations o ON o.id=? WHERE p.id=?`).get(observationId??null,positionId) as {external_id:string|null;canonical_url:string;title:string|null;location:string|null}|null;
    if(!row)return "missing";
    return createHash("sha256").update(JSON.stringify({externalId:row.external_id,url:row.canonical_url,title:row.title,location:row.location})).digest("hex");
  }
  ...
  reconcileGig(processing:string|ScoutPositionProcessingJob,now:string){
    const processingId=typeof processing==="string"?processing:(processing.processingId??processing.id);
    this.db.transaction(()=>{
      const work=this.db.query(`SELECT position_id positionId,run_id runId,observation_id observationId,configuration_source_id configurationSourceId FROM scout_position_processing WHERE id=? AND stage='reconcile_gig' AND status='pending'`).get(processingId) as {positionId:string;runId:string|null;observationId:string|null;configurationSourceId:string|null}|null;
      if(!work)return;
      const observationId=work.observationId;
      if(!observationId)throw new Error("Scout position observation not found.");
      const observation=this.db.query(`SELECT 1 FROM scout_position_observations WHERE id=? AND position_id=?`).get(observationId,work.positionId);
      if(!observation)throw new Error("Scout position observation not found.");
      const descriptionIdentity=this.positionBackfillIdentity(work.runId,this.descriptionIdentity(work.positionId,observationId,work.configurationSourceId??undefined));
      this.ensureStage(work.positionId,"acquire_description",descriptionIdentity,now,{runId:work.runId??undefined,observationId,configurationSourceId:work.configurationSourceId??undefined});
      this.db.query(`UPDATE scout_position_processing SET status='completed',attempt_count=attempt_count+1,failure_code=NULL,failure_message=NULL,updated_at=?,completed_at=? WHERE id=? AND status='pending'`).run(now,now,processingId);
    })();
  }
  ...
  backfillPositions(sourceRunId:string,limit:number,now:string){return this.db.transaction(()=>{
    ...
    const backfillRunId=execution.id,checkpointName=`reconcile_gig:${sourceRunId}`;
    const checkpoint=this.db.query(`SELECT last_position_id,completed_at FROM scout_position_backfill WHERE name=? AND source_run_id=?`).get(checkpointName,sourceRunId) as {last_position_id:string|null;completed_at:string|null}|null;
    const completed=Boolean(checkpoint?.completed_at),startAfter=checkpoint?.last_position_id??"";
  ...
  private resolvePositionBackfill(command:ScoutPositionBackfillCommand){
    ...
    for(const positionId of normalized.positionIds){
      const position=this.db.query(`SELECT p.id positionId,p.external_id externalId,p.canonical_url canonicalUrl,(SELECT run_company.company_name FROM scout_position_observations observation JOIN scout_run_sources run_source ON run_source.id=observation.run_source_id JOIN scout_run_companies run_company ON run_company.id=run_source.run_company_id WHERE observation.position_id=p.id ORDER BY observation.observed_at DESC,observation.id DESC LIMIT 1) company,p.title,coalesce(s.state,'processing') state,s.linked_gig_id linkedGigId,(SELECT decision.origin FROM scout_position_decisions decision WHERE decision.id=s.current_decision_id) initialDecisionOrigin FROM scout_positions p LEFT JOIN scout_position_states s ON s.position_id=p.id WHERE p.id=?`).get(positionId) as {positionId:string;externalId:string|null;canonicalUrl:string;company:string;title:string;state:ScoutPositionState;linkedGigId:string|null;initialDecisionOrigin:"agent"|"user"|"system"|null}|null;
  ...
  descriptionVerificationCandidates(){return this.db.query(`SELECT x.id processingId,rc.company_name company,o.canonical_url sourceUrl,s.settings_json settings FROM scout_position_processing x JOIN scout_position_observations o ON o.id=x.observation_id JOIN scout_run_sources rs ON rs.id=o.run_source_id JOIN scout_run_companies rc ON rc.id=rs.run_company_id JOIN scout_company_configuration_sources s ON s.id=rs.configuration_source_id WHERE x.stage='acquire_description' AND x.status='failed' AND x.failure_code='description_too_large' ORDER BY rc.company_name,x.updated_at DESC`).all() as Array<{processingId:string;company:string;sourceUrl:string;settings:string}>;}
  ...
  descriptionInput(processingId:string):ScoutDescriptionInput{const row=this.db.query(`SELECT r.run_type runType,x.description_id preparedDescriptionId,p.id positionId,p.external_id externalId,o.title,rc.company_name company,o.location,o.canonical_url officialUrl,s.settings_json settings,d.id existingDescriptionId FROM scout_position_processing x JOIN scout_runs r ON r.id=x.run_id JOIN scout_positions p ON p.id=x.position_id JOIN scout_position_observations o ON o.id=x.observation_id JOIN scout_run_sources rs ON rs.id=o.run_source_id JOIN scout_run_companies rc ON rc.id=rs.run_company_id JOIN scout_company_configuration_sources s ON s.id=coalesce(x.configuration_source_id,rs.configuration_source_id) LEFT JOIN scout_position_descriptions d ON d.position_id=p.id AND d.artifact_id=o.description_artifact_id WHERE x.id=? AND x.stage='acquire_description' AND x.status IN ('pending','failed')`).get(processingId) as Record<string,unknown>|null;if(!row)throw new Error("Scout description work not found.");const source=JSON.parse(String(row.settings)) as SourceConfiguration;const position={id:nullableText(row.externalId),title:String(row.title),url:String(row.officialUrl)},preparedDescriptionId=nullableText(row.preparedDescriptionId),existingDescriptionId=preparedDescriptionId??(row.runType==="position_backfill"?null:nullableText(row.existingDescriptionId));return{positionId:String(row.positionId),externalId:position.id,title:position.title,company:String(row.company),location:nullableText(row.location),officialUrl:position.url,existingDescriptionId,detailPlan:existingDescriptionId?null:this.detailPlan(source,position)};}
  ...
  relevanceInput(processingId:string):RelevanceRequest&{confidenceThreshold:number}{const row=this.db.query(`SELECT x.run_id runId,p.id positionId,o.title,run_company.company_name company,o.location,o.canonical_url officialUrl,d.id descriptionId,d.artifact_id descriptionArtifactId,d.markdown_content_hash descriptionHash,a.file_path filePath,criteria.criteria,criteria.version criteriaVersion,criteria.confidence_threshold confidenceThreshold,criteria.prompt_version promptVersion FROM scout_position_processing x JOIN scout_positions p ON p.id=x.position_id JOIN scout_position_observations o ON o.id=x.observation_id JOIN scout_run_sources run_source ON run_source.id=o.run_source_id JOIN scout_run_companies run_company ON run_company.id=run_source.run_company_id JOIN scout_position_descriptions d ON d.id=x.description_id JOIN scout_description_artifacts a ON a.id=d.artifact_id JOIN scout_relevance_criteria criteria ON criteria.id=x.criteria_id WHERE x.id=? AND x.status='pending'`).get(processingId) as Record<string,unknown>|null;if(!row||typeof row.runId!=="string"||!this.screeningSnapshot(row.runId)||!this.descriptionsRoot)throw new Error("Scout relevance inputs are unavailable.");return{positionId:String(row.positionId),title:String(row.title),company:String(row.company),location:nullableText(row.location),officialUrl:String(row.officialUrl),descriptionMarkdown:readFileSync(path.resolve(this.descriptionsRoot,String(row.filePath)),"utf8"),descriptionArtifactId:String(row.descriptionArtifactId),descriptionHash:String(row.descriptionHash),criteria:String(row.criteria),criteriaVersion:Number(row.criteriaVersion),confidenceThreshold:Number(row.confidenceThreshold)/1000,promptVersion:String(row.promptVersion)};}
  ...
  candidateMatchInput(processingId:string):CandidateMatchRequest{
    ...
    const row=this.db.query(`SELECT sr.id runId,p.id positionId,o.title,run_company.company_name company,o.location,o.canonical_url officialUrl,d.artifact_id descriptionArtifactId,d.markdown_content_hash descriptionHash,a.file_path filePath,re.id relevanceEvaluationId,r.rubric,r.version rubricVersion,r.prompt_version promptVersion
      ...
      JOIN scout_run_sources run_source ON run_source.id=o.run_source_id
      JOIN scout_run_companies run_company ON run_company.id=run_source.run_company_id
  ...
  positions(
    ...
    const where = ["rc.run_id = ?"],
      params: string[] = [runId];
    if (input.company) {
      where.push("lower(rc.company_name) LIKE ? ESCAPE '\\'");
      params.push(
    ...
    const from = ` FROM scout_position_observations o JOIN scout_run_sources rs ON rs.id=o.run_source_id JOIN scout_run_companies rc ON rc.id=rs.run_company_id WHERE ${where.join(" AND ")}`;
    const total = Number(
      (
    ...
    const items = this.db
      .query(
        `SELECT o.id,o.title,rc.company_name company,o.canonical_url canonicalUrl,o.location,o.observed_at observedAt,rs.status sourceStatus,o.description_artifact_id descriptionArtifactId,o.provenance_json provenance${from} ORDER BY lower(rc.company_name),lower(o.title),o.id LIMIT ? OFFSET ?`,
      )
  ...
  positionDetail(positionId:string){return this.positionDetailForCurrentPosting(positionId,false);}
  promotionPositionDetail(positionId:string){return this.positionDetailForCurrentPosting(positionId,true);}
  private positionDetailForCurrentPosting(positionId:string,includeLinkedGig:boolean):ScoutPositionDetail|null{const linkedGigFilter=includeLinkedGig?"":"AND s.linked_gig_id IS NULL";const row=this.db.query(`SELECT p.id,p.title,(SELECT run_company.company_name FROM scout_position_observations observation JOIN scout_run_sources run_source ON run_source.id=observation.run_source_id JOIN scout_run_companies run_company ON run_company.id=run_source.run_company_id WHERE observation.position_id=p.id ORDER BY observation.observed_at DESC,observation.id DESC LIMIT 1) company,p.location,p.canonical_url canonicalUrl,p.external_id externalId,p.source_key sourceKey,s.state,s.revision stateRevision,p.first_seen_at firstSeenAt,p.last_seen_at lastSeenAt,(SELECT count(*) FROM scout_position_observations o WHERE o.position_id=p.id) observationCount,EXISTS(SELECT 1 FROM scout_position_observations o WHERE o.position_id=p.id AND o.description_artifact_id IS NOT NULL) descriptionAvailable,(SELECT stage FROM scout_position_processing x WHERE x.position_id=p.id AND x.status<>'superseded' ORDER BY x.created_at DESC,x.id DESC LIMIT 1) processingStage,(SELECT status FROM scout_position_processing x WHERE x.position_id=p.id AND x.status<>'superseded' ORDER BY x.created_at DESC,x.id DESC LIMIT 1) processingStatus,(SELECT failure_code FROM scout_position_processing x WHERE x.position_id=p.id AND x.status<>'superseded' ORDER BY x.created_at DESC,x.id DESC LIMIT 1) processingFailureCode,(SELECT failure_message FROM scout_position_processing x WHERE x.position_id=p.id AND x.status<>'superseded' ORDER BY x.created_at DESC,x.id DESC LIMIT 1) processingFailureMessage,(SELECT m.score FROM scout_candidate_match_evaluations m WHERE m.position_id=p.id ORDER BY m.created_at DESC,m.id DESC LIMIT 1) score,(SELECT m.score_explanation FROM scout_candidate_match_evaluations m WHERE m.position_id=p.id ORDER BY m.created_at DESC,m.id DESC LIMIT 1) scoreExplanation,(SELECT rc.version FROM scout_relevance_evaluations re JOIN scout_relevance_criteria rc ON rc.id=re.criteria_id WHERE re.position_id=p.id ORDER BY re.created_at DESC,re.id DESC LIMIT 1) criteriaVersion,(SELECT r.version FROM scout_candidate_match_evaluations m JOIN scout_candidate_match_rubrics r ON r.id=m.rubric_id WHERE m.position_id=p.id ORDER BY m.created_at DESC,m.id DESC LIMIT 1) rubricVersion,(SELECT m.profile_version FROM scout_candidate_match_evaluations m WHERE m.position_id=p.id ORDER BY m.created_at DESC,m.id DESC LIMIT 1) profileVersion,(SELECT m.model FROM scout_candidate_match_evaluations m WHERE m.position_id=p.id ORDER BY m.created_at DESC,m.id DESC LIMIT 1) model,(SELECT m.provider FROM scout_candidate_match_evaluations m WHERE m.position_id=p.id ORDER BY m.created_at DESC,m.id DESC LIMIT 1) provider FROM scout_positions p JOIN scout_position_states s ON s.position_id=p.id WHERE p.id=? AND s.state NOT IN ('rejected','irrelevant') ${linkedGigFilter}`).get(positionId) as (Omit<ScoutPositionDetail,"observations"|"descriptionAvailable"|"observationCount">&{descriptionAvailable:number;observationCount:number})|null;if(!row)return null;const observations=(this.db.query(`SELECT o.id,rc.run_id runId,r.created_at runCreatedAt,rc.status companyStatus,cs.source_key sourceKey,rs.status sourceStatus,o.title,o.canonical_url canonicalUrl,o.location,o.observed_at observedAt,o.description_artifact_id IS NOT NULL descriptionAvailable,o.provenance_json provenance FROM scout_position_observations o JOIN scout_run_sources rs ON rs.id=o.run_source_id JOIN scout_company_configuration_sources cs ON cs.id=rs.configuration_source_id JOIN scout_run_companies rc ON rc.id=rs.run_company_id JOIN scout_runs r ON r.id=rc.run_id WHERE o.position_id=? ORDER BY o.observed_at DESC,o.id DESC`).all(positionId) as Array<Omit<ScoutPositionDetail["observations"][number],"descriptionAvailable"|"provenance">&{descriptionAvailable:number;provenance:string}>).map(value=>({...value,descriptionAvailable:Boolean(value.descriptionAvailable),provenance:JSON.parse(value.provenance) as unknown}));return{...row,descriptionAvailable:Boolean(this.db.query(`SELECT 1 FROM scout_position_descriptions WHERE position_id=? LIMIT 1`).get(positionId)),observationCount:Number(row.observationCount),score:row.score===null?null:Number(row.score),observations} as ScoutPositionDetail;}
  reviewDetail(positionId:string):ScoutPositionDetail|null{
    return this.reviewDetailForCurrentPosting(positionId,false);
  }
  private reviewDetailForCurrentPosting(positionId:string,includeLinkedGig:boolean):ScoutPositionDetail|null{
    const base=this.positionDetailForCurrentPosting(positionId,includeLinkedGig);if(!base)return null;
    ...
    const promotion=this.db.query(`SELECT promotion.status,promotion.failure_code failureCode,promotion.failure_message failureMessage FROM scout_position_promotions promotion JOIN scout_position_states state ON state.position_id=promotion.position_id AND state.current_decision_id=promotion.decision_id WHERE promotion.position_id=?`).get(positionId) as {status:"pending"|"completed"|"failed";failureCode:string|null;failureMessage:string|null}|null;
  ...
  private reviewedPosting(positionId:string,observationId:string,descriptionId:string):Omit<ScoutPostingReview,"detail">|null{
    const source=this.db.query(`SELECT p.identity_kind identityKind,p.identity_value identityValue,rc.company_name company,o.title,o.canonical_url canonicalUrl,o.location,o.provenance_json observationProvenance,cs.source_key sourceKey,cs.company_configuration_id configurationVersionId,configuration.version configurationVersion,d.id descriptionId,d.source_url sourceUrl,d.retrieved_at retrievedAt,d.source_content_hash sourceContentHash,d.markdown_content_hash extractedContentHash,d.converter_version converterVersion,a.file_path filePath,a.provenance_json descriptionProvenance FROM scout_position_observations o JOIN scout_positions p ON p.id=o.position_id JOIN scout_run_sources rs ON rs.id=o.run_source_id JOIN scout_run_companies rc ON rc.id=rs.run_company_id JOIN scout_company_configuration_sources cs ON cs.id=rs.configuration_source_id JOIN scout_company_configurations configuration ON configuration.id=cs.company_configuration_id JOIN scout_position_descriptions d ON d.id=? AND d.position_id=p.id JOIN scout_description_artifacts a ON a.id=d.artifact_id WHERE o.id=? AND o.position_id=?`).get(descriptionId,observationId,positionId) as Record<string,unknown>|null;
    ...
    const extractionStrategy=nullableText(descriptionProvenance.extractionStrategy)??nullableText(descriptionProvenance.strategyVersion)??"listing-description-v1";
    const converterVersion=nullableText(descriptionProvenance.converterVersion)??nullableText(source.converterVersion);
    if(!converterVersion)throw new Error("Reviewed Scout description converter provenance is unavailable.");
    const sourceDescription=JSON.stringify({scoutDescriptionId:String(source.descriptionId),officialSourceUrl:String(source.sourceUrl),retrievedAt:String(source.retrievedAt),sourceKey:String(source.sourceKey),configurationVersionId:String(source.configurationVersionId),extractionStrategy,converterVersion});
    ...
    return{
      ...
      markdown,
      sourceDescription,
      sourceProvenance:{officialUrl:String(source.sourceUrl),retrievedAt:String(source.retrievedAt),sourceContentHash:String(source.sourceContentHash),extractedContentHash:String(source.extractedContentHash),sourceKey:String(source.sourceKey),configurationVersion:Number(source.configurationVersion),extractionStrategy,converterVersion},
    };
  }
  private currentPosting(positionId:string):ScoutPostingReview|null{
    const detail=this.reviewDetailForCurrentPosting(positionId,true);
    if(!detail||!detail.descriptionId||!detail.relevanceEvaluationId||!detail.candidateMatchEvaluationId)return null;
    const binding=this.db.query(`SELECT x.observation_id observationId,r.description_id descriptionId FROM scout_candidate_match_evaluations m JOIN scout_relevance_evaluations r ON r.id=m.relevance_evaluation_id JOIN scout_position_processing x ON x.position_id=m.position_id AND x.stage='score_candidate_match' AND x.status='completed' AND x.input_identity=m.input_identity AND x.relevance_evaluation_id=r.id WHERE m.id=? AND m.position_id=? AND x.observation_id IS NOT NULL`).get(detail.candidateMatchEvaluationId,positionId) as {observationId:string;descriptionId:string}|null;
  ...
  reviewPosting(positionId:string):ScoutPostingReview|null{
    const current=this.currentPosting(positionId);
    return current?.detail.state==="needs_user_review"?current:null;
  }
  ...
  private exactPromotionWork(positionId:string):ScoutPromotionWork|null{
    ...
    return{kind:"attempt",positionId,descriptionId:String(source.descriptionId),changeId:String(source.changeId),actor:String(source.actor),resolution:resolution.data,...reviewed};
  }
  ...
  promotionWork(positionId:string){
    const attempt=this.exactPromotionWork(positionId);
    if(attempt)return attempt;
    const completed=this.db.query(`SELECT state.linked_gig_id linkedGigId FROM scout_position_states state JOIN scout_position_promotions promotion ON promotion.position_id=state.position_id AND promotion.status='completed' AND promotion.gig_id=state.linked_gig_id WHERE state.position_id=? AND state.state='promoted' AND state.linked_gig_id IS NOT NULL ORDER BY promotion.completed_at DESC,promotion.id DESC LIMIT 1`).get(positionId) as {linkedGigId:string}|null;
    if(!completed)return null;
    const current=this.currentPosting(positionId);
    if(!current)return null;
    return{kind:"completed_retry" as const,positionId,descriptionId:current.detail.descriptionId!,actor:"Gig Scout",linkedGigId:completed.linkedGigId,observationId:current.observationId,posting:current.posting,markdown:current.markdown,sourceDescription:current.sourceDescription,sourceProvenance:current.sourceProvenance};
  }
  releasePromotion(positionId:string,changeId:string,outcome:"resolution_stale"|"resolution_invalid",now:string){
    return this.db.transaction(()=>{
      const attempt=this.db.query(`SELECT promotion.id,decision.actor,state.revision FROM scout_position_promotions promotion JOIN scout_position_decisions decision ON decision.id=promotion.decision_id JOIN scout_position_states state ON state.position_id=promotion.position_id AND state.current_decision_id=promotion.decision_id WHERE promotion.position_id=? AND decision.change_id=? AND promotion.status IN ('pending','failed')`).get(positionId,changeId) as {id:string;actor:string;revision:number}|null;
      if(!attempt)throw new Error("Scout promotion is no longer pending.");
      this.db.query(`UPDATE scout_position_promotions SET status='failed',failure_code=?,failure_message=NULL,attempt_count=attempt_count+1,updated_at=? WHERE id=?`).run(outcome,now,attempt.id);
      this.db.query(`INSERT INTO scout_position_state_history(change_id,operation,recorded_at,recorded_by,position_id,state,linked_gig_id,deferred_until,revision,created_at,updated_at,current_decision_id) SELECT ?,'update',?,?,position_id,state,linked_gig_id,deferred_until,revision,created_at,updated_at,current_decision_id FROM scout_position_states WHERE position_id=?`).run(changeId,now,attempt.actor,positionId);
      this.db.query(`UPDATE scout_position_states SET state='needs_user_review',linked_gig_id=NULL,deferred_until=NULL,current_decision_id=NULL,revision=revision+1,updated_at=? WHERE position_id=? AND revision=?`).run(now,positionId,attempt.revision);
      const detail=this.reviewDetail(positionId);
      if(!detail)throw new Error("Released Scout position could not be reviewed.");
      return detail;
    })();
  }
  failPromotion(positionId:string,message:string,now:string){this.db.query(`UPDATE scout_position_promotions SET status='failed',failure_code='promotion_failed',failure_message=?,attempt_count=attempt_count+1,updated_at=? WHERE position_id=? AND decision_id=(SELECT current_decision_id FROM scout_position_states WHERE position_id=?) AND status<>'completed'`).run(message.slice(0,500),now,positionId,positionId);}
  completePromotion(positionId:string,gigId:string,managedDocumentId:string,now:string){this.db.transaction(()=>{const completed=this.db.query(`UPDATE scout_position_promotions SET gig_id=?,managed_document_id=?,status='completed',failure_code=NULL,failure_message=NULL,attempt_count=attempt_count+1,updated_at=?,completed_at=? WHERE position_id=? AND decision_id=(SELECT current_decision_id FROM scout_position_states WHERE position_id=?) AND status IN ('pending','failed')`).run(gigId,managedDocumentId,now,now,positionId,positionId);if(completed.changes!==1)throw new Error("Scout promotion is no longer pending.");this.db.query(`UPDATE scout_position_states SET state='promoted',linked_gig_id=?,deferred_until=NULL,updated_at=? WHERE position_id=?`).run(gigId,now,positionId);})();}
