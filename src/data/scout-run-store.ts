...
export class SqliteScoutRunStore implements ScoutRunStore,ScoutPositionStore,ScoutPositionProcessingRepository {
  ...
  backfillStatus(runId:string):ScoutPositionBackfillStatus|null{
    ...
    const projections=this.db.query(`SELECT i.position_id positionId,x.document_projection_status documentProjectionStatus FROM scout_position_backfill_items i LEFT JOIN scout_position_processing x ON x.run_id=i.run_id AND x.position_id=i.position_id AND x.stage='acquire_description' WHERE i.run_id=? AND i.linked_gig_id IS NOT NULL ORDER BY i.position_id`).all(runId) as Array<{positionId:string;documentProjectionStatus:ScoutPromotedDescriptionOutcome|"pending"|"failed"|null}>;
