...
export class SqliteScoutRunStore implements ScoutRunStore,ScoutPositionStore,ScoutPositionProcessingRepository {
  ...
  private promote(positionId:string,decisionId:string,descriptionId:string,changeId:string,actor:string,now:string){
    ...
    if(!source||!this.descriptionsRoot)throw new Error("Reviewed Scout description is unavailable.");
    const markdown=readFileSync(path.resolve(this.descriptionsRoot,String(source.filePath)),"utf8"),gigId=id("gig","scout",positionId),documentId=id("doc","scout",descriptionId),promotionId=id("spprom",positionId);
    this.db.query(`INSERT OR IGNORE INTO gigs(id,company,title,external_job_id,stage,outcome,status_summary,last_activity,fit_rating,source_url,location,tags_json,has_job_description,has_interview_prep,revision,is_deleted,created_at,updated_at) VALUES(?,?,?,?,'identified','pending','Promoted from Gig Scout',?,'unknown',?,?, '[]',1,0,1,0,?,?)`).run(gigId,String(source.company),String(source.title),nullableText(source.externalId),now.slice(0,10),String(source.sourceUrl??source.canonicalUrl),nullableText(source.location),now,now);
    this.db.query(`INSERT OR IGNORE INTO managed_documents(id,document_type,title,media_type,source_description,upload_provenance_json,current_version,created_at,updated_at) VALUES(?,'job_description',?,'text/markdown','Exact reviewed Gig Scout description',?,1,?,?)`).run(documentId,`${source.company} — ${source.title}`,JSON.stringify({scoutPositionId:positionId,scoutDescriptionId:descriptionId,officialSourceUrl:source.sourceUrl,retrievedAt:source.retrievedAt,sourceProvenance:JSON.parse(String(source.provenance))}),now,now);
    this.db.query(`INSERT OR IGNORE INTO managed_document_versions(document_id,version,parent_version,content,content_hash,change_id,change_summary,created_at,created_by) VALUES(?,1,NULL,?,?,?,'Promoted exact Scout description',?,?)`).run(documentId,markdown,String(source.contentHash),changeId,now,actor);
    this.db.query(`INSERT OR IGNORE INTO managed_document_links(document_id,gig_id) VALUES(?,?)`).run(documentId,gigId);
  ...
  retryPromotion(positionId:string,now:string):ScoutPositionDetail|null{const promotion=this.db.query(`SELECT p.decision_id decisionId,p.description_id descriptionId,d.change_id changeId,d.actor FROM scout_position_promotions p JOIN scout_position_decisions d ON d.id=p.decision_id WHERE p.position_id=? AND p.status IN ('pending','failed')`).get(positionId) as {decisionId:string;descriptionId:string;changeId:string;actor:string}|null;if(!promotion)return this.reviewDetail(positionId);try{this.db.transaction(()=>this.promote(positionId,promotion.decisionId,promotion.descriptionId,promotion.changeId,promotion.actor,now))();}catch(reason){const message=reason instanceof Error?reason.message:"Promotion failed.";this.db.query(`UPDATE scout_position_promotions SET status='failed',failure_code='promotion_failed',failure_message=?,attempt_count=attempt_count+1,updated_at=? WHERE position_id=? AND status<>'completed'`).run(message.slice(0,500),now,positionId);}return this.reviewDetail(positionId);}
