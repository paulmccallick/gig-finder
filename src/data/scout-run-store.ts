...
export class SqliteScoutRunStore implements ScoutRunStore,ScoutPositionStore,ScoutPositionProcessingRepository {
  ...
  startBackfill(command:ScoutPositionBackfillCommand,now:string):ScoutPositionBackfillStatus{return this.db.transaction(()=>{
    ...
    for(const item of resolved.accepted){
      this.db.query(`INSERT INTO scout_position_backfill_items(run_id,position_id,observation_id,configuration_source_id,linked_gig_id,requested_at) VALUES(?,?,?,?,?,?)`).run(runId,item.positionId,item.observationId,item.configurationSourceId,item.linkedGigId,now);
  ...
  backfillStatus(runId:string):ScoutPositionBackfillStatus|null{
    ...
    const projections=this.db.query(`SELECT i.position_id positionId,x.id processingId,x.status,x.failure_code failureCode FROM scout_position_backfill_items i LEFT JOIN scout_position_processing x ON x.run_id=i.run_id AND x.position_id=i.position_id AND x.stage='acquire_description' WHERE i.run_id=? AND i.linked_gig_id IS NOT NULL ORDER BY i.position_id`).all(runId) as Array<{positionId:string;processingId:string|null;status:string|null;failureCode:string|null}>;
    for(const projection of projections){
      if(projection.status==="failed"||projection.failureCode?.startsWith("document_projection_")){gigDocuments.failed++;continue;}
      if(projection.status!=="completed"||!projection.processingId){gigDocuments.pending++;continue;}
      const documentChangeId=id("change","promoted-description",projection.processingId);
      const updated=this.db.query(`SELECT 1 FROM managed_document_versions WHERE change_id=? LIMIT 1`).get(documentChangeId);
      if(updated)gigDocuments.updated++;else gigDocuments.unchanged++;
    }
    return{runId,reason:run.reason,selection:{requested:accepted,accepted,rejected:0},stages,positionOutcomes,gigDocuments};
  }
  ...
  prepareDescriptionCompletion(processingId:string,value:ScoutDescriptionResult,now:string):{descriptionId:string;promotedDocument:ScoutPromotedDescriptionWork|null}{return this.db.transaction(()=>{
    ...
    let descriptionId=work.descriptionId;
    ...
    return{descriptionId,promotedDocument:this.promotedDescriptionWork(processingId,descriptionId)};
  })();}
  ...
  failDescriptionProjection(processingId:string,code:string,message:string,now:string){const boundedCode=code.trim().replace(/[^a-z0-9_]+/gi,"_").slice(0,70)||"failed";this.db.query(`UPDATE scout_position_processing SET attempt_count=attempt_count+1,failure_code=?,failure_message=?,updated_at=?,completed_at=NULL WHERE id=? AND stage='acquire_description' AND status='pending' AND description_id IS NOT NULL`).run(`document_projection_${boundedCode}`.slice(0,100),message.slice(0,500),now,processingId);}
  private promotedDescriptionWork(processingId:string,descriptionId:string):ScoutPromotedDescriptionWork|null{
    const row=this.db.query(`SELECT x.id processingId,x.position_id positionId,i.linked_gig_id gigId,prom.managed_document_id managedDocumentId,document.current_version expectedDocumentVersion,d.source_url officialUrl,d.retrieved_at retrievedAt,d.source_content_hash sourceContentHash,d.converter_version converterVersion,artifact.file_path filePath,artifact.provenance_json provenance,source.source_key sourceKey,configuration.version configurationVersion FROM scout_position_processing x JOIN scout_runs run ON run.id=x.run_id AND run.run_type='position_backfill' JOIN scout_position_backfill_items i ON i.run_id=x.run_id AND i.position_id=x.position_id JOIN scout_position_states state ON state.position_id=x.position_id AND state.linked_gig_id=i.linked_gig_id JOIN scout_position_promotions prom ON prom.position_id=x.position_id AND prom.status='completed' AND prom.gig_id=i.linked_gig_id AND prom.managed_document_id IS NOT NULL JOIN managed_documents document ON document.id=prom.managed_document_id JOIN scout_position_descriptions d ON d.id=x.description_id AND d.id=? AND d.position_id=x.position_id JOIN scout_description_artifacts artifact ON artifact.id=d.artifact_id JOIN scout_company_configuration_sources source ON source.id=x.configuration_source_id JOIN scout_company_configurations configuration ON configuration.id=source.company_configuration_id WHERE x.id=? AND i.linked_gig_id IS NOT NULL`).get(descriptionId,processingId) as Record<string,unknown>|null;
    if(!row)return null;
    if(!this.descriptionsRoot)throw new Error("Scout description artifact storage is unavailable.");
    const provenance=JSON.parse(String(row.provenance)) as Record<string,unknown>;
    const extractedContentHash=typeof provenance.extractedContentHash==="string"?provenance.extractedContentHash:"";
    const extractionStrategy=typeof provenance.strategyVersion==="string"?provenance.strategyVersion:"";
