...
export class SqliteScoutRunStore implements ScoutRunStore,ScoutPositionStore,ScoutPositionProcessingRepository {
  ...
  private promote(positionId:string,descriptionId:string,changeId:string,actor:string,now:string){
    const source=this.db.query(`SELECT p.title,p.location,p.external_id externalId,p.canonical_url canonicalUrl,p.source_key positionSourceKey,c.name company,d.source_url sourceUrl,d.retrieved_at retrievedAt,d.markdown_content_hash contentHash,a.file_path filePath,a.provenance_json provenance,(SELECT cs.source_key FROM scout_position_processing x JOIN scout_position_observations o ON o.id=x.observation_id JOIN scout_run_sources rs ON rs.id=o.run_source_id JOIN scout_company_configuration_sources cs ON cs.id=rs.configuration_source_id WHERE x.description_id=d.id ORDER BY x.created_at DESC,x.id DESC LIMIT 1) authoritativeSourceKey,(SELECT cs.company_configuration_id FROM scout_position_processing x JOIN scout_position_observations o ON o.id=x.observation_id JOIN scout_run_sources rs ON rs.id=o.run_source_id JOIN scout_company_configuration_sources cs ON cs.id=rs.configuration_source_id WHERE x.description_id=d.id ORDER BY x.created_at DESC,x.id DESC LIMIT 1) configurationVersionId FROM scout_positions p JOIN scout_companies c ON c.id=p.company_id JOIN scout_position_descriptions d ON d.id=? AND d.position_id=p.id JOIN scout_description_artifacts a ON a.id=d.artifact_id WHERE p.id=?`).get(descriptionId,positionId) as Record<string,unknown>|null;
