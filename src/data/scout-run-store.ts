...
export class SqliteScoutRunStore implements ScoutRunStore,ScoutPositionStore {
  ...
  backfillPositions(limit:number,now:string){return this.db.transaction(()=>{const checkpoint=this.db.query(`SELECT last_position_id FROM scout_position_backfill WHERE name='reconcile_gig'`).get() as {last_position_id:string|null}|null;const rows=this.db.query(`SELECT id FROM scout_positions WHERE id>? ORDER BY id LIMIT ?`).all(checkpoint?.last_position_id??"",limit) as Array<{id:string}>;let created=0;for(const row of rows){this.initializePosition(row.id,now);if(this.ensurePositionProcessing(row.id,now))created++;}const complete=rows.length<limit;const last=complete?null:(rows.at(-1)?.id??null);this.db.query(`INSERT INTO scout_position_backfill(name,last_position_id,completed_at,updated_at) VALUES('reconcile_gig',?,?,?) ON CONFLICT(name) DO UPDATE SET last_position_id=excluded.last_position_id,completed_at=excluded.completed_at,updated_at=excluded.updated_at`).run(last,complete?now:null,now);return{created,complete};})();}
  ...
  positionDetail(positionId:string):ScoutPositionDetail|null{const item=this.workspace({sort:"last_seen",direction:"desc",offset:0,limit:100,state:"actionable"}).items.find(value=>value.id===positionId)??this.workspace({sort:"last_seen",direction:"desc",offset:0,limit:100,state:"irrelevant"}).items.find(value=>value.id===positionId);if(!item)return null;const identity=this.db.query(`SELECT external_id externalId,source_key sourceKey FROM scout_positions WHERE id=?`).get(positionId) as {externalId:string|null;sourceKey:string};const observations=(this.db.query(`SELECT o.id,rc.run_id runId,r.created_at runCreatedAt,rc.status companyStatus,cs.source_key sourceKey,rs.status sourceStatus,o.title,o.canonical_url canonicalUrl,o.location,o.observed_at observedAt,o.description_artifact_id IS NOT NULL descriptionAvailable,o.provenance_json provenance FROM scout_position_observations o JOIN scout_run_sources rs ON rs.id=o.run_source_id JOIN scout_company_configuration_sources cs ON cs.id=rs.configuration_source_id JOIN scout_run_companies rc ON rc.id=rs.run_company_id JOIN scout_runs r ON r.id=rc.run_id WHERE o.position_id=? ORDER BY o.observed_at DESC,o.id DESC`).all(positionId) as Array<Record<string,unknown>>).map(row=>({...row,descriptionAvailable:Boolean(row.descriptionAvailable),provenance:JSON.parse(String(row.provenance))}));return{...item,...identity,observations} as ScoutPositionDetail;}
}
