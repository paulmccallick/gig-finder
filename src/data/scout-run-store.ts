...
export class SqliteScoutRunStore implements ScoutRunStore,ScoutPositionStore {
  ...
  backfillPositions(limit:number,now:string){return this.db.transaction(()=>{const gigIdentitySignature=createHash("sha256").update(JSON.stringify(this.db.query(`SELECT company,external_job_id,source_url,is_deleted FROM gigs ORDER BY id`).all())).digest("hex");const checkpoint=this.db.query(`SELECT last_position_id,gig_identity_signature,completed_at FROM scout_position_backfill WHERE name='reconcile_gig'`).get() as {last_position_id:string|null;gig_identity_signature:string|null;completed_at:string|null}|null;if(checkpoint?.completed_at&&checkpoint.gig_identity_signature===gigIdentitySignature)return{created:0,complete:true};const identityChanged=Boolean(checkpoint?.completed_at&&checkpoint.gig_identity_signature!==gigIdentitySignature),startAfter=identityChanged?"":(checkpoint?.last_position_id??"");const rows=this.db.query(`SELECT id FROM scout_positions WHERE id>? ORDER BY id LIMIT ?`).all(startAfter,limit) as Array<{id:string}>;let created=0;for(const row of rows){this.initializePosition(row.id,now);if(this.ensurePositionProcessing(row.id,now))created++;}const complete=rows.length<limit;const last=rows.at(-1)?.id??(identityChanged?null:checkpoint?.last_position_id??null);this.db.query(`INSERT INTO scout_position_backfill(name,last_position_id,gig_identity_signature,completed_at,updated_at) VALUES('reconcile_gig',?,?,?,?) ON CONFLICT(name) DO UPDATE SET last_position_id=excluded.last_position_id,gig_identity_signature=excluded.gig_identity_signature,completed_at=excluded.completed_at,updated_at=excluded.updated_at`).run(last,gigIdentitySignature,complete?now:null,now);return{created,complete};})();}
