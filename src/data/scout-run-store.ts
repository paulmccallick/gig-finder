...
export class SqliteScoutRunStore implements ScoutRunStore,ScoutPositionStore,ScoutPostingResolutionStore,ScoutPositionProcessingRepository {
  ...
  releasePromotion(positionId:string,changeId:string,outcome:"resolution_stale"|"resolution_invalid",now:string){this.db.transaction(()=>{const attempt=this.db.query(`SELECT promotion.id,decision.actor,state.revision FROM scout_position_promotions promotion JOIN scout_position_decisions decision ON decision.id=promotion.decision_id JOIN scout_position_states state ON state.position_id=promotion.position_id AND state.current_decision_id=promotion.decision_id WHERE promotion.position_id=? AND decision.change_id=? AND promotion.status IN ('pending','failed')`).get(positionId,changeId) as {id:string;actor:string;revision:number}|null;if(!attempt)throw new Error("Scout promotion is no longer pending.");this.db.query(`UPDATE scout_position_promotions SET status='failed',failure_code=?,failure_message=NULL,attempt_count=attempt_count+1,updated_at=? WHERE id=?`).run(outcome,now,attempt.id);this.db.query(`INSERT INTO scout_position_state_history(change_id,operation,recorded_at,recorded_by,position_id,state,linked_gig_id,deferred_until,revision,created_at,updated_at,current_decision_id) SELECT ?,'update',?,?,position_id,state,linked_gig_id,deferred_until,revision,created_at,updated_at,current_decision_id FROM scout_position_states WHERE position_id=?`).run(changeId,now,attempt.actor,positionId);this.db.query(`UPDATE scout_position_states SET state='needs_user_review',linked_gig_id=NULL,deferred_until=NULL,current_decision_id=NULL,revision=revision+1,updated_at=? WHERE position_id=? AND revision=?`).run(now,positionId,attempt.revision);})();}
