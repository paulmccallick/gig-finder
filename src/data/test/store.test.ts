...
describe("change idempotency and reversal", () => {
  ...
  test("creation retries are bound to their original payload",()=>{const artifacts={jobDescription:async()=>"",interviewPrep:async()=>[],jobDescriptionExists:async()=>false,interviewPrepExists:async()=>false,verify:async()=>({ok:true,errors:[],unregistered:[]})} satisfies ArtifactPort;const app=new GigFinderApplication(store,new AuditReader(database),artifacts);const createContext={actor:"Candidate",source:"agent" as const,summary:"Create gig",changeId:"agent-tool:create-gig"};const input={company:"Retry Company",title:"Director",externalJobId:null,stage:"identified" as const,outcome:"pending" as const,statusSummary:"Identified",lastActivity:"2026-08-04",nextAction:null,fit:{rating:"good" as const,summary:null},payRange:null,sourceUrl:null,tags:[],location:null,workArrangement:null,postedDate:null,businessUnitTeam:null,recruiterSource:null,bonus:null,equity:null,otherCompensation:null};const created=app.gigs.createNew(createContext,"gig-created",input);expect(app.gigs.createNew(createContext,"gig-created",input)).toEqual(created);expect(()=>app.gigs.createNew(createContext,"gig-created",{...input,title:"Different"})).toThrow(new MutationError("revision_conflict","Change agent-tool:create-gig does not match Gig gig-created and payload."));app.gigs.update({actor:"Candidate",source:"test",summary:"later"},"gig-created",{statusSummary:"Later edit"});expect(app.gigs.createNew(createContext,"gig-created",input)).toMatchObject({changeId:createContext.changeId,record:{statusSummary:"Later edit"}});expect(()=>app.people.createNew(createContext,"person-with-gig-change",{name:"Other"})).toThrow(new MutationError("revision_conflict","Change agent-tool:create-gig does not match Person person-with-gig-change and payload."));const personContext={...createContext,changeId:"agent-tool:create-person"};const createdPerson=app.people.createNew(personContext,"person-created",{name:"Taylor",company:null});expect(app.people.createNew(personContext,"person-created",{name:"Taylor",company:null})).toEqual(createdPerson);expect(()=>app.people.createNew(personContext,"person-created",{name:"Different",company:null})).toThrow(new MutationError("revision_conflict","Change agent-tool:create-person does not match Person person-created and payload."));app.people.update({actor:"Candidate",source:"test",summary:"later"},"person-created",{title:"Updated"});expect(app.people.createNew(personContext,"person-created",{name:"Taylor",company:null})).toMatchObject({changeId:personContext.changeId,record:{title:"Updated"}});const relationshipContext={...createContext,changeId:"agent-tool:create-relationship"};const relationshipInput={gigId:"gig-created",personId:"person-created",relationship:"recruiter" as const,notes:null};const relationship=app.gigPeople.createNew(relationshipContext,"relationship-created",relationshipInput);expect(app.gigPeople.createNew(relationshipContext,"relationship-created",relationshipInput)).toEqual(relationship);expect(()=>app.gigPeople.createNew(relationshipContext,"relationship-created",{...relationshipInput,notes:"Different"})).toThrow(new MutationError("revision_conflict","Change agent-tool:create-relationship does not match relationship relationship-created and payload."));});

  test("recreates a reverted relationship tuple with a new ID",()=>{const artifacts={jobDescription:async()=>"",interviewPrep:async()=>[],jobDescriptionExists:async()=>false,interviewPrepExists:async()=>false,verify:async()=>({ok:true,errors:[],unregistered:[]})} satisfies ArtifactPort;const app=new GigFinderApplication(store,new AuditReader(database),artifacts);app.gigs.createNew({actor:"test",source:"test",summary:"gig"},"gig",{company:"Example",title:"Director",externalJobId:null,stage:"identified",outcome:"pending",statusSummary:"Found",lastActivity:"2026-08-05",nextAction:null,fit:{rating:"good",summary:null},payRange:null,sourceUrl:null,tags:[],location:null,workArrangement:null,postedDate:null,businessUnitTeam:null,recruiterSource:null,bonus:null,equity:null,otherCompensation:null});app.people.createNew({actor:"test",source:"test",summary:"person"},"person",{name:"Taylor"});const first=app.gigPeople.createNew({actor:"test",source:"agent",summary:"first",changeId:"relationship:first"},"relationship-first",{gigId:"gig",personId:"person",relationship:"recruiter",notes:null});app.changes.revert({actor:"test",source:"test",summary:"undo",changeId:"relationship:undo"},first.changeId!);const second=app.gigPeople.createNew({actor:"test",source:"agent",summary:"second",changeId:"relationship:second"},"relationship-second",{gigId:"gig",personId:"person",relationship:"recruiter",notes:null});expect(second.record.id).toBe("relationship-second");expect(app.gigPeople.list()).toHaveLength(1);expect(()=>app.gigPeople.createNew({actor:"test",source:"agent",summary:"duplicate",changeId:"relationship:duplicate"},"relationship-third",{gigId:"gig",personId:"person",relationship:"recruiter",notes:null})).toThrow(new MutationError("duplicate","Relationship already exists: relationship-second"));});
