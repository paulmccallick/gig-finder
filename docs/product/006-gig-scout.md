...
- **Promotion Resolution**: Pursue records the reviewed posting resolution before mutating a Gig. If candidate identity changes during that handoff, Scout terminalizes that exact attempt, preserves its audited decision and resolution, clears it from the current state projection, and returns the position to `needs_user_review` for a new reviewed change. Gig or document infrastructure failures retain the exact current attempt for deterministic retry.
