...
- **Explicit Position Reprocessing**: An operator may preview and start a bounded rerun for reviewed exact position IDs and a reason. Preview accepts only positions whose current immutable source configuration can authoritatively reacquire that exact position's description; listing-only configurations without a detail acquisition plan are rejected before work starts. The rerun preserves completed descriptions, evaluations, decisions, processing records, and managed-document versions as immutable history while replacing current projections only as each new stage succeeds. It refetches the official description and intentionally reruns relevance and eligible candidate scoring even when the normalized Markdown identity is unchanged.
